...
function pytest_loop {
    ...
    while true; do
        clr_green "*** PYTEST **************************************************************************************"
        /opt/python3/bin/python3 -m pytest "${my_dir}" --disable-warnings
        ...
        /opt/python3/bin/python3 -m mypy "${my_dir}" --strict --no-warn-unused-ignores --follow-imports=skip
            # shellcheck disable=SC2181  # Check Exit Code directly
            if [[ "${?}" -gt 0 ]]; then
        ...
        /opt/python3/bin/python3 -m mypy "${my_dir}" --strict --no-warn-unused-ignores &>/dev/null
            # shellcheck disable=SC2181  # Check Exit Code directly
            if [[ "${?}" -gt 0 ]]; then
